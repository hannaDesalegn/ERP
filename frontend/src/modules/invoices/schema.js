/**
 * Invoice validation — B owns this file.
 *
 * ── COPYING THIS MODULE ──────────────────────────────────────────────────────
...
/**
 * Create. Server-managed fields (id, invoiceNumber, lines[].productName/sku/
 * unitPrice/lineTotal, subtotal, taxTotal, grandTotal, amountDue, status
 * 'overdue', createdAt, updatedAt) are deliberately absent — the client must
 * not be able to set them, and the backend must reject them if sent
 * (mass-assignment protection).
 *
 * `amountDue` in particular is never a form field anywhere — it is always
 * grandTotal - amountPaid, computed server-side.
 */
export const invoiceCreateSchema = z.object({
  ...
  issueDate: z.string().min(1, 'Pick an issue date.'), // YYYY-MM-DD
  dueDate: z.string().min(1, 'Pick a due date.'),

  lines: z.array(lineItemSchema).min(1, 'Add at least one line item.'),

  // The user can record a payment amount when creating/editing; the server
  // recomputes amountDue from it. Never send amountDue itself.
  amountPaid: z
    .number()
  ...
  // 'overdue' excluded on purpose — it is server-derived, never chosen.
...
/** @typedef {z.infer<typeof invoiceCreateSchema>} InvoiceInput */